...
// Match any token
export const anyToken = (): Parser<Token> => {
  return (tokens: Token[]) => {
    if (tokens.length === 0) {
      return {
        success: false,
...
export const parseAll = <T>(parser: Parser<T>): Parser<T> => {
  return (tokens: Token[]) => {
    const result = parser(tokens);
    if (!result.success) {
      return result;
...
export const keyword = (value: string): Parser<Token> => token('KEYWORD', value);
export const operator = (value: string): Parser<Token> => token('OPERATOR', value);
export const punctuation = (value: string): Parser<Token> => token('PUNCTUATION', value);
