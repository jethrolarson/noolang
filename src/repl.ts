...
export class REPL {
  ...
  private processInput(input: string): void {
    ...
    try {
      ...
      console.log(`${colorize.section('➡')} ${colorize.value(formatValue(programResult.finalResult))} \t ${colorize.section(':')} ${type ? colorize.type(this.typeToString(type)) : 'unknown'}`);
      ...
      if (programResult.executionTrace.length > 1) {
        ...
        programResult.executionTrace.forEach((step, i) => {
          console.log(`  ${colorize.number(`${i + 1}.`)} ${colorize.identifier(step.expression)} ${colorize.operator('→')} ${colorize.value(formatValue(step.result))}`);
        });
      }
    } catch (error) {
      console.error(colorize.error(`Error: ${(error as Error).message}`));
  ...
  private showHelp(): void {
    ...
    console.log(`  ${colorize.command('.env')}      - Show current environment`);
    console.log(`  ${colorize.command('.env-detail')} - Show detailed environment with types`);
    console.log(`  ${colorize.command('.env-json')} - Show environment as JSON`);
    ...
    console.log(`  ${colorize.command('.tokens (expr)')}     - Show tokens for expression`);
    console.log(`  ${colorize.command('.tokens-file file')}  - Show tokens for file`);
    console.log(`  ${colorize.command('.ast (expr)')}        - Show AST for expression`);
    console.log(`  ${colorize.command('.ast-file file')}     - Show AST for expression`);
    console.log(`  ${colorize.command('.ast-json (expr)')}   - Show AST as JSON`);
    console.log(`  ${colorize.command('.error-detail')}      - Show detailed error info`);
    console.log(`  ${colorize.command('.error-context')}     - Show error context`);
    console.log('');
  ...
  private showEnvironment(): void {
    ...
    for (const [name, value] of env) {
      console.log(`  ${colorize.identifier(name)}: ${colorize.value(formatValue(value))}`);
    }
  }
  ...
  private showTokensFromParens(input: string): void {
    ...
    if (!match) {
      console.log(colorize.warning('Usage: .tokens (expression) - expression must be wrapped in parentheses'));
      return;
    }
    ...
    try {
      ...
      console.log(`${colorize.section('Tokens for')} "${colorize.identifier(expression)}":`);
      tokens.forEach((token, i) => {
        console.log(`  ${colorize.number(`${i}:`)} ${colorize.type(token.type)} ${colorize.string(`'${token.value}'`)}`);
      });
    } catch (error) {
      console.error(colorize.error(`Error tokenizing: ${(error as Error).message}`));
    }
  }
  ...
  private showTokens(input: string): void {
    ...
    try {
      ...
      console.log(`${colorize.section('Tokens for')} "${colorize.identifier(input)}":`);
      tokens.forEach((token, i) => {
        console.log(`  ${colorize.number(`${i}:`)} ${colorize.type(token.type)} ${colorize.string(`'${token.value}'`)}`);
      });
    } catch (error) {
  ...
  private showTokensFromFile(filename: string): void {
    ...
    try {
      ...
      console.log(`${colorize.section('Tokens for file')} "${colorize.identifier(filename)}":`);
      ...
      console.error(colorize.error(`Error reading file: ${(error as Error).message}`));
    }
  }
  ...
  private showASTFromParens(input: string): void {
    ...
    const match = input.match(/^\((.*)\)$/);
    if (!match) {
      console.log(colorize.warning('Usage: .ast (expression) - expression must be wrapped in parentheses'));
      return;
    }
    ...
    try {
      ...
      console.log(`${colorize.section('AST for')} "${colorize.identifier(expression)}":`);
      console.log(this.astToString(program.statements[0]));
    } catch (error) {
      console.error(colorize.error(`Error parsing: ${(error as Error).message}`));
    }
  }
  ...
  private showAST(input: string): void {
    ...
    try {
      ...
      console.log(`${colorize.section('AST for')} "${colorize.identifier(input)}":`);
      console.log(this.astToString(program.statements[0]));
    } catch (error) {
      console.error(colorize.error(`Error parsing: ${(error as Error).message}`));
    }
  ...
  private showASTFromFile(filename: string): void {
    ...
    try {
      ...
      console.log(`${colorize.section('AST for file')} "${colorize.identifier(filename)}":`);
      console.log(this.astToString(program.statements[0]));
    } catch (error) {
      console.error(colorize.error(`Error parsing file: ${(error as Error).message}`));
    }
  }
  ...
  private showASTAsJSONFromParens(input: string): void {
    ...
    const match = input.match(/^\((.*)\)$/);
    if (!match) {
      console.log(colorize.warning('Usage: .ast-json (expression) - expression must be wrapped in parentheses'));
      return;
    }
  ...
  private showErrorDetail(): void {
    console.log(colorize.warning('Error detail not available - no recent error'));
  }

  private showErrorContext(): void {
    console.log(colorize.warning('Error context not available - no recent error'));
  }
  ...
  private astToString(expr: any, indent: number = 0): string {
    ...
    switch (expr.kind) {
      case 'literal':
        return `${spaces}${colorize.type('Literal')}: ${colorize.value(expr.value)}`;
      case 'variable':
        return `${spaces}${colorize.type('Variable')}: ${colorize.identifier(expr.name)}`;
      case 'function':
        return `${spaces}${colorize.type('Function')}:\n${spaces}  ${colorize.section('params')}: [${expr.params.map((p: string) => colorize.identifier(p)).join(', ')}]\n${spaces}  ${colorize.section('body')}: ${this.astToString(expr.body, indent + 1)}`;
      case 'application':
        return `${spaces}${colorize.type('Application')}:\n${spaces}  ${colorize.section('func')}: ${this.astToString(expr.func, indent + 1)}\n${spaces}  ${colorize.section('args')}: [${expr.args.map((arg: any) => this.astToString(arg, indent + 1)).join(', ')}]`;
      case 'binary':
        return `${spaces}${colorize.type('Binary')}(${colorize.operator(expr.operator)}):\n${spaces}  ${colorize.section('left')}: ${this.astToString(expr.left, indent + 1)}\n${spaces}  ${colorize.section('right')}: ${this.astToString(expr.right, indent + 1)}`;
      case 'definition':
        return `${spaces}${colorize.type('Definition')}:\n${spaces}  ${colorize.section('name')}: ${colorize.identifier(expr.name)}\n${spaces}  ${colorize.section('value')}: ${this.astToString(expr.value, indent + 1)}`;
      case 'import':
        return `${spaces}${colorize.type('Import')}: ${colorize.string(`"${expr.path}"`)}`;
      case 'record':
        return `${spaces}${colorize.type('Record')}:\n${expr.fields.map((field: any) => `${spaces}  ${colorize.identifier(field.name)}: ${this.astToString(field.value, indent + 1)}`).join('\n')}`;
      case 'accessor':
        return `${spaces}${colorize.type('Accessor')}: ${colorize.operator('@')}${colorize.identifier(expr.field)}`;
      default:
        return `${spaces}${colorize.type(expr.kind)}: ${colorize.value(JSON.stringify(expr))}`;
    }
  }
