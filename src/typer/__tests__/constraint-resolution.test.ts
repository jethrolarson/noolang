...
describe.skip('Constraint Resolution (LEGACY - REMOVED)', () => {
	test('should resolve constraint function calls', () => {
		...
		const showSignature: ConstraintSignature = {
			name: 'Show',
			typeParams: ['a'],
			functions: new Map([['show', functionType([intType()], stringType())]]),
		};
	...
	test('should handle multiple constraints and implementations', () => {
		...
		const showResolution = tryResolveConstraintFunction(
			'show',
			[],
			[intType()],
			state
		);
		const eqResolution = tryResolveConstraintFunction(
			'equals',
			[],
			[intType(), intType()],
			state
