...
const unificationConstraintsEqual = (
	c1: UnificationConstraint,
	c2: UnificationConstraint
): boolean => {
	...
	switch (c1.kind) {
		case 'equal':
			...
			return (
				...
				c1.type2.kind === c2Equal.type2.kind &&
...
export class ConstraintSolver {
	...
	addConstraint(constraint: UnificationConstraint): void {
		...
		if (
			this.state.constraints.some(existing =>
				unificationConstraintsEqual(constraint, existing)
			)
		) {
	...
	solve(): {
		...
		try {
			...
			if (solveTime > 10) {
				console.warn(
					`Slow constraint solve: ${solveTime}ms for ${this.state.constraints.length} constraints`
				);
			}
			return {
	...
	private solveConstraints(): void {
		...
		if (iterations >= maxIterations) {
			this.state.errors.push(
				`Constraint solving exceeded maximum iterations (${iterations}) - possible infinite constraint loop. Started with ${initialConstraintCount} constraints, ${this.state.constraints.length} remaining.`
			);
		}
		...
		if (this.state.constraints.length > 0) {
			for (const constraint of this.state.constraints) {
				this.state.errors.push(
					`Unsolved constraint: ${JSON.stringify(constraint)}`
				);
			}
		}
	}
	...
	private processConstraint(constraint: UnificationConstraint): boolean {
		switch (constraint.kind) {
			case 'equal':
				return this.processEqualConstraint(
					constraint.type1,
					constraint.type2,
					constraint.location
				);
			case 'instance':
				return this.processInstanceConstraint(
					constraint.typeVar,
					constraint.type,
					constraint.location
				);
			case 'check':
				return this.processCheckConstraint(
			...
			default:
				this.state.errors.push(
					`Unknown constraint kind: ${(constraint as any).kind}`
				);
				return true; // Remove unknown constraints
		}
	}

	private processEqualConstraint(
		type1: Type,
		type2: Type,
		location?: { line: number; column: number }
	): boolean {
	...
	private processInstanceConstraint(
		...
		if (this.occursIn(typeVar, type)) {
			this.state.errors.push(
				`Occurs check failed: ${typeVar} occurs in ${this.typeToString(type)}`
			);
			return true;
		}
		...
		if (currentSub) {
			...
			if (!this.typesEqual(currentSub, substitutedType)) {
				this.state.errors.push(
					`Type variable ${typeVar} has conflicting substitutions: ${this.typeToString(currentSub)} vs ${this.typeToString(substitutedType)}`
				);
			}
		...
		const equivalenceClass =
			this.state.unionFind.getEquivalenceClasses().get(root) || [];
			...
			if (otherVar !== typeVar) {
				...
				if (otherSub) {
					...
					if (!this.typesEqual(otherSub, substitutedType)) {
						this.state.errors.push(
							`Type variable ${otherVar} has conflicting substitutions: ${this.typeToString(otherSub)} vs ${this.typeToString(substitutedType)}`
						);
					}
				} else {
	...
	private decomposeTypes(
		...
		if (type1.kind !== type2.kind) {
			this.state.errors.push(
				`Type mismatch: cannot unify ${this.typeToString(type1)} with ${this.typeToString(type2)}`
			);
			return true;
		}
		...
		switch (type1.kind) {
			case 'function':
				...
				if (type1.params.length !== type2.params.length) {
					this.state.errors.push(
						`Function arity mismatch: ${type1.params.length} vs ${type2.params.length}`
					);
					return true;
				}
				...
				for (let i = 0; i < type1.params.length; i++) {
					this.addConstraint({
						kind: 'equal',
						type1: type1.params[i],
						type2: type2.params[i],
						location,
					});
				}
				this.addConstraint({
			...
			case 'tuple':
				...
				if (type1.elements.length !== type2.elements.length) {
					this.state.errors.push(
						`Tuple length mismatch: ${type1.elements.length} vs ${type2.elements.length}`
					);
					return true;
				}
				...
				for (let i = 0; i < type1.elements.length; i++) {
					this.addConstraint({
						kind: 'equal',
						type1: type1.elements[i],
						type2: type2.elements[i],
						location,
					});
				}
				return true;
			...
			case 'record':
				...
				for (const key of keys1) {
					...
					this.addConstraint({
						kind: 'equal',
						type1: type1.fields[key],
						type2: type2.fields[key],
						location,
					});
				}
			...
			case 'primitive':
				...
				if (type1.name !== type2.name) {
					this.state.errors.push(
						`Type mismatch: expected ${type1.name} but got ${type2.name}`
					);
			...
			case 'variant':
				...
				if (type1.name !== type2.name) {
					this.state.errors.push(
						`Variant name mismatch: ${type1.name} vs ${type2.name}`
					);
					return true;
				}
				...
				if (type1.args.length !== type2.args.length) {
					this.state.errors.push(
						`Variant arity mismatch: ${type1.name} has ${type1.args.length} vs ${type2.args.length} args`
					);
					return true;
				}
			...
			default:
				this.state.errors.push(
					`Unknown type kind for decomposition: ${type1.kind}`
				);
				return true;
		}
	}
	...
	private typesEqual(type1: Type, type2: Type): boolean {
		...
		switch (t1.kind) {
			case 'variable':
				...
				return (
					t1.name === t2.name ||
					this.state.unionFind.areUnified(t1.name, t2.name)
				);
			case 'primitive':
	...
	private occursIn(typeVar: string, type: Type): boolean {
		switch (type.kind) {
			...
			case 'function':
				return (
					type.params.some(p => this.occursIn(typeVar, p)) ||
